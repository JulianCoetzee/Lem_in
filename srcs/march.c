...
void    ant_march(char ***paths, int ant_total)
{
    ...
    move_out->ants = (int *)ft_memalloc(sizeof(int) * move_out->route_total + 1);
    if (move_out->route_total == 1 || ant_total == 1)
        single_file(paths[0], ant_total, move_out->path_sizes[0]);
    else
    {
        if (!(move_out->route_total = ant_routes_used(ant_total, move_out->path_sizes)))
            single_file(paths[0], ant_total, move_out->path_sizes[0]);
        else
        {
...
void    ant_columns(t_march *move_out)
{
    ...
    {
        while (move_out->ants[i] + move_out->path_sizes[i] < move_out->path_sizes[rt])
        {
            move_out->ants[i]++;
            ants--;
        }
